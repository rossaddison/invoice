...
/**
 * Shared requirement-check list consumed by both the CLI `requirements.php`
 * script and the web-based `/install` wizard, so the two never drift apart.
 */
final class RequirementsConfig
{
    /**
     * @return list<array{name: string, mandatory: bool, condition: bool, by: string, memo: string}>
     */
    public static function checks(): array
    {
        ...
        return [
            ...
            [
                ...
                'mandatory' => true,
                // Note: checkPhpIniOn() only recognizes boolean-style ini values
                // ('on'/1) — it's the wrong check for a size string like "1024M"
                // and would always evaluate false here, so it's deliberately not used.
                'condition' => ini_get('memory_limit') === '-1'
                    || $requirementsChecker->getByteSize((string) ini_get('memory_limit')) >= $requirementsChecker->getByteSize('1024M'),
