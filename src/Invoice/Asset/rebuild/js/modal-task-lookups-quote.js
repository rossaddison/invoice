(function () {
    ...
    function updateSelectTaskButtonState(root) {
        ...
        document.querySelectorAll('.select-items-confirm-task-quote').forEach(function (btn) {
            if (anyChecked) {
    ...
    function handleSelectItemsConfirmTaskQuote(btn) {
        ...
        fetch('/invoice/task/selection_quote?' + params.toString(), {
            method: 'GET',
            ...
            .catch(function (err) {
                ...
                btn.disabled = false;
                alert('An error occurred while adding tasks to quote. See console for details.');
            });
    }
    ...
    document.addEventListener('click', function (e) {
        ...
        var confirmTask = el.closest('.select-items-confirm-task-quote');
        if (confirmTask) {
            handleSelectItemsConfirmTaskQuote(confirmTask);
            return;
        }

        // Reset / load actions: these can be adapted if project loads the table via URL
        if (el.closest('#task-reset-button-quote')) {
            var task_table = document.querySelector('#tasks_table');
            if (task_table) {
                var lookup_url = location.origin + "/invoice/task/lookup";
                task_table.innerHTML = '<h2 class="text-center"><i class="fa fa-spin fa-spinner"></i></h2>';
                lookup_url += "?rt=true";
                setTimeout(function () {
                    ...
                    fetch(lookup_url, { cache: 'no-store', credentials: 'same-origin' })
                        ...
                        .then(function (html) { 
                            ...
                            task_table.appendChild(fragment);
                            updateSelectTaskButtonState(task_table); 
                        })
                        .catch(function (err) { console.error('task lookup load failed', err); });
                }, 50);
            }
            // enable button after reset
            document.querySelectorAll('.select-items-confirm-task-quote').forEach(function (b) { b.removeAttribute('disabled'); });
            return;
        }
    }, true);
