...
/**
 * Maps a raw XPath path string to a Scala property-access expression.
 *
 * Produces the same structural results as TypeScriptVoPathMapper but with
 * idiomatic Scala dot-access (no optional-chaining `?.`):
 *   cbc:PascalCase  → v.camelCase
 *   cac:Singular    → v.camelCase           (plain dot; VO fields assumed non-null)
 *   cac:Plural      → v.camelCases          (Seq)
 *   cac:P/cbc:X     → v.ps.map(e => e.x)
...
final class ScalaVoPathMapper
{
    ...
    /**
     * True when the path resolves to a Seq property (0..* aggregate element).
     * Used by the expression emitter to choose between .nonEmpty and != null.
     */
    ...
    private function mapNextStep(string $step, array $remainingSteps, string $base): string
    {
        ...
        if ($prefix === 'cac' && in_array($local, self::PLURAL, true)) {
            ...
            return $remainingSteps === []
                ? $arrayExpr
                : $arrayExpr . '.map(e => ' . $this->mapSteps($remainingSteps, 'e') . ')';
        }

        // Both cac: (objects) and cbc: (scalars) use plain dot in Scala
        return $this->mapSteps($remainingSteps, $base . '.' . $camel);
    }
