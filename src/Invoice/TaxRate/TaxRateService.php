...
final readonly class TaxRateService
{
    ...
    {
        isset($array['tax_rate_name']) ?
        $model->setTaxRateName((string) $array['tax_rate_name']) : '';
        isset($array['tax_rate_percent']) ?
        $model->setTaxRatePercent((float) $array['tax_rate_percent']) : '';
        isset($array['tax_rate_code']) ?
        $model->setTaxRateCode((string) $array['tax_rate_code']) : '';
        $model->setTaxRateDefault($array['tax_rate_default'] === '1' ?
        true : false);
        isset($array['peppol_tax_rate_code']) ?
