...
final class QuoteController extends BaseController
{
    ...
    public function add(
        ...
        if (('main' != $origin) && ('quote' != $origin) && ('dashboard' != $origin)) {
            $layoutParameters = [
                'type'              => 'client',
                'form'              => $layoutWithForm,
                'return_url_action' => 'add',
                ...
                if (is_array($body)) {
                    ...
                    if (null !== $user) {
                        ...
                        if (null !== $model_id) {
                            ...
                            $this->flashMessage('info', '1' === $this->sR->getSetting('generate_quote_number_for_draft')
    ...
    public function create_confirm(Request $request, FormHydrator $formHydrator, GR $gR, TRR $trR, UR $uR, UCR $ucR, UIR $uiR): \Yiisoft\DataResponse\DataResponse
    {
        ...
        $ajax_body = [
            ...
            'number'           => '1' === $this->sR->getSetting('generate_quote_number_for_draft') ? $gR->generate_number((int) $body['quote_group_id'], true) : '',
        ...
        if ($formHydrator->populate($ajax_content, $ajax_body) && $ajax_content->isValid()) {
            ...
            if (null !== $user_client && 1 == $user_client_count) {
                ...
                if (null !== $user) {
                    ...
                    if (null !== $user_inv && $user_inv->getActive()) {
                        ...
                        if (null !== $model_id) {
                            ...
                            $this->flashMessage(
                                'info',
                                '1' === $this->sR->getSetting('generate_quote_number_for_draft')
                                  ? $this->translator->translate('generate_quote_number_for_draft').'=>'.$this->translator->translate('yes')
                                  : $this->translator->translate('generate_quote_number_for_draft').'=>'.$this->translator->translate('no'),
                            );
    ...
    public function default_taxes(Quote $quote, TRR $trR, FormHydrator $formHydrator): void
    {
        if ($trR->repoCountAll() > 0) {
            ...
            foreach ($taxrates as $taxrate) {
                1 == $taxrate->getTaxRateDefault() ? $this->default_tax_quote($taxrate, $quote, $formHydrator) : '';
            }
        }
    }
    ...
    public function default_tax_quote(?TaxRate $taxrate, Quote $quote, FormHydrator $formHydrator): void
    {
         ...
         */
        $quote_tax_rate['include_item_tax']      = ('1' == $this->sR->getSetting('default_include_item_tax') ? 1 : 0);
    ...
    public function edit_save_custom_fields(array|object|null $parse, FormHydrator $formHydrator, QCR $qcR, ?string $quote_id): void
    {
        /**
         * @var array $custom
         */
        ...
        foreach ($custom as $custom_field_id => $value) {
            if (0 == $qcR->repoQuoteCustomCount((string) $quote_id, (string) $custom_field_id)) {
                $quoteCustom        = new QuoteCustom();
                $quote_custom_input = [
        ...
        if (null !== $quote_id) {
            ...
            if ($quote_entity) {
                ...
                if ($pdf_template_target_path) {
                     ...
                     * @var string $from[1]
                     */
                    $mail_from = [$template_helper->parse_template($quote_id, false, $from[0], $cR, $cvR, $iR, $iaR, $qR, $qaR, $soR, $uiR),
    ...
    public function generate_quote_number_if_applicable(string $quote_id, QR $qR, SR $sR, GR $gR): void
    {
        ...
        if (!empty($quote) && (1 == $quote->getStatus_id()) && ('' == $quote->getNumber())) {
            // Generate new quote number if applicable
            if (0 == $sR->getSetting('generate_quote_number_for_draft')) {
    ...
    public function pdf(#[RouteArgument('include')] int $include, CR $cR, CVR $cvR, CFR $cfR, GR $gR, QAR $qaR, QCR $qcR, QIR $qiR, QIAR $qiaR, QR $qR, QTRR $qtrR, SR $sR, UIR $uiR): \Yiisoft\DataResponse\DataResponse|Response
    {
        ...
        if ($quote_amount) {
            ...
            if (1 == $sR->getSetting('mark_quotes_sent_pdf')) {
            ...
            if ($quote) {
                ...
                $parameters = ('1' == $include ? ['success' => 1] : ['success' => 0]);
    ...
    private function quote_to_invoice_quote_custom(
        ...
        foreach ($quote_customs as $quote_custom) {
            ...
            if (0 !== $cfR->repoTableAndLabelCountquery('inv_custom', (string) $existing_custom_field->getLabel())) {
    ...
    private function quote_to_quote_quote_amount(string $quote_id, ?string $copy_id): void
    {
    ...
    public function url_key(#[RouteArgument('url_key')] string $urlKey, CurrentUser $currentUser, CFR $cfR, QAR $qaR, QIR $qiR, QIAR $qiaR, QR $qR, QTRR $qtrR, UIR $uiR, UCR $ucR, PMR $pmR): Response
    {
        ...
        if ($quote) {
            ...
            if (in_array($quote->getStatus_id(), [2, 3, 4, 5, 6])) {
                ...
                if (1 === $uiR->repoUserInvUserIdcount($user_id)) {
                    ...
                    $user_client = 1 === $ucR->repoUserClientqueryCount($user_id, $quote->getClient_id()) ? true : false;
                    if ($user_inv && $user_client) {
                        ...
                        if (1 == $user_inv->getType()) {
                            if (2 === $quote->getStatus_id()) {
