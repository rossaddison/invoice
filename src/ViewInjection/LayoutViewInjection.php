...
final readonly class LayoutViewInjection implements LayoutParametersInjectionInterface
{
    ...
    public function getLayoutParameters(): array
    {
        ...
        $bootstrap5OffcanvasEnable             = '1' == $this->settingRepository->getSetting('bootstrap5_offcanvas_enable') ? true : false;
        ...
        $stopSigningUp                         = '1' == $this->settingRepository->getSetting('stop_signing_up') ? true : false;
        $stopLoggingIn                         = '1' == $this->settingRepository->getSetting('stop_logging_in') ? true : false;
        $noFrontPageAbout                      = '1' == $this->settingRepository->getSetting('no_front_about_page') ? true : false;
        $noFrontPageGallery                    = '1' == $this->settingRepository->getSetting('no_front_gallery_page') ? true : false;
        $noFrontPageAccreditations             = '1' == $this->settingRepository->getSetting('no_front_accreditations_page') ? true : false;
        $noFrontPageTeam                       = '1' == $this->settingRepository->getSetting('no_front_team_page') ? true : false;
        $noFrontPagePricing                    = '1' == $this->settingRepository->getSetting('no_front_pricing_page') ? true : false;
        $noFrontPageTestimonial                = '1' == $this->settingRepository->getSetting('no_front_testimonial_page') ? true : false;
        $noFrontPagePrivacyPolicy              = '1' == $this->settingRepository->getSetting('no_front_privacy_policy_page') ? true : false;
        $noFrontPageTermsOfService             = '1' == $this->settingRepository->getSetting('no_front_terms_of_service') ? true : false;
        $noFrontPageContactDetails             = '1' == $this->settingRepository->getSetting('no_front_contact_details_page') ? true : false;
        $noFrontPageContactUs                  = '1' == $this->settingRepository->getSetting('no_front_contact_us_page') ? true : false;
        /**
         * @see .env.php $_ENV['YII_DEBUG'] and $_ENV['BUILD_DATABASE'] located in the root (first) folder
         *      e.g YII_DEBUG=true
         * @see {root} autoload.php
         */
        $debugMode     = 'true' == $_ENV['YII_DEBUG'] ? true : false;
        $buildDatabase = 'true' == $_ENV['BUILD_DATABASE'] ? true : false;
