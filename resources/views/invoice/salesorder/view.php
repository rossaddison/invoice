...
?>
        ...
        <div class="options btn-group">
            ...
            if ($invEdit) { ?> 
                <li>
                    <a href="<?php echo $urlGenerator->generate('salesorder/edit', ['id' => $so->getId()]); ?>" style="text-decoration:none">
            ...
            if (null !== $so->getInv_id() || in_array($so->getStatus_id(), [1, 2, 3, 4, 5])) {
            } else {?> 
                    <?php if ($invEdit) { ?> 
                        <li>
                            <a href="#so-to-invoice" data-bs-toggle="modal"  style="text-decoration:none">
                                <i class="fa fa-refresh fa-margin"></i>
                                <?php echo $translator->translate('salesorder.to.invoice'); ?>
                            </a>
                        </li>
                    <?php } ?>    
                <?php } ?>
        ...
        <div class="salesorder">
            <div class = 'row'>
                <div class="col-xs-12 col-sm-6 col-md-5">
                    <h3>
                        <a href="<?php echo $urlGenerator->generate('client/view', ['id' => $so->getClient()?->getClient_id()]); ?>">
                            <?php echo Html::encode($clienthelper->format_client($so->getClient())); ?>
                        </a>
                    </h3>
                    <br>
...
echo null !== $soCountry ? '<br>'.$countryhelper->get_country_name($translator->translate('cldr'), $soCountry) : ''; ?>
                            ...
                            <div class="col-xs-12 col-md-6">
                                ...
                                <div>
                                    <label for="salesorder_number">
                                        <?php echo $translator->translate('salesorder'); ?> #
                                    </label>
                                    <input type="text" id="salesorder_number" class="form-control" readonly
                                        <?php if (null !== $so->getNumber()) { ?> value="<?php echo $so->getNumber(); ?>"
                                        <?php } else { ?> placeholder="<?php echo $translator->translate('not.set'); ?>"
                                        <?php } ?>>
                                </div>
                                <div has-feedback">
                                    <label for="salesorder_date_created">
                                        <?php echo '0' == $vat ? $translator->translate('date.issued') : $translator->translate('salesorder.date.created'); ?>
                                    </label>
                                    <div class="input-group">
                                        <input name="salesorder_date_created" id="salesorder_date_created" disabled
                                               class="form-control"
                                               value="<?php echo Html::encode($so->getDate_created() instanceof DateTimeImmutable ?
                                ...
                                <?php if ($invNumber) { ?>  
                                <div has-feedback">
                                    <label for="salesorder_to_url"><?php echo $translator->translate('salesorder.invoice'); ?></label>
                                    <div class="input-group">
                                        <?php echo Html::a($invNumber, $urlGenerator->generate('inv/view', ['id' => $so->getInv_id()]), ['class' => 'btn btn-success']); ?>
                                    </div>
                                </div>
                                <?php } ?>
                                <div>
                            ...
                            <div class="col-xs-12 col-md-6">
                                ...
                                <div>
                                    <label for="salesorder_password" hidden>
                                        <?php echo $translator->translate('salesorder.password'); ?>
                                    </label>
                                    <input type="text" id="salesorder_password" class="form-control" disabled value="<?php echo Html::encode($body['password'] ?? ''); ?>" hidden>
                                </div>
                                <div>
                                    <label for="salesorder_client_purchase_order_number">
                                        <?php echo $translator->translate('salesorder.clients.purchase.order.number'); ?>
                                    </label>
                                    <input type="text" id="salesorder_client_purchase_order_number" class="form-control" disabled value="<?php echo Html::encode($body['client_po_number'] ?? ''); ?>">
                                </div>
                                <div>
                                    <label for="salesorder_client_purchase_order_person">
                                        <?php echo $translator->translate('salesorder.clients.purchase.order.person'); ?>
                                    </label>
                                    <input type="text" id="salesorder_client_purchase_order_number" class="form-control" disabled value="<?php echo Html::encode($body['client_po_person'] ?? ''); ?>">
                                </div>
                                    ...
                                    <?php
                                        ...
                                        <div>
                                            <br>
                                            <a href="<?php echo $urlGenerator->generate('salesorder/url_key', ['key' => $so->getUrl_key()]); ?>" class="btn btn-success">  
                                                <?php echo $translator->translate('salesorder.agree.to.terms').'/'.$translator->translate('salesorder.reject'); ?>    
                                            </a>
                                        </div>
                                    <?php } ?>
                                <input type="text" id="dropzone_client_id" readonly  hidden class="form-control" value="<?php echo $so->getClient()?->getClient_id(); ?>">
                            </div>
                        </div>
                    </div>
                </div>
   ...
   <div id="partial_item_table_parameters" quote_items="<?php ?>" disabled>
                ...
                <div class="panel panel-default no-margin">
                    ...
                    <div class="panel-body">
                        <textarea name="notes" id="notes" rows="3" disabled
                                  class="input-sm form-control"><?php echo Html::encode($so->getNotes() ?? ''); ?></textarea>
                    </div>
                </div>
                <br>
