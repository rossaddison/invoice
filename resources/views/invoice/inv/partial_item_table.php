                    ...
                    <td class="td-amount ">
                        <div class="input-group">
                            ...
                            <input disabled type="text"
                                   ...
                                   class="input-sm form-control amount"
                                   data-bs-toggle = "tooltip"
                                   title="inv_item->discount_amount"
                                   value="<?= $numberHelper->format_amount(
...
<?php // Buttons for line item start here?>
                    <td class="td-vert-middle btn-group">
                        <?php if ($showButtons === true && $userCanEdit === true
                                && $draft === true) { ?>
                            <?php if ($piR->repoCount(
                                    (int) $item->getProduct_id()) > 0) { ?>
                ...
                <?php
                    ...
                    foreach ($productImages as $productImage) { ?>
                        <?php if (!empty($productImage->getFile_name_original())) { ?> 
                            ...
                            <a  href="
        <?= $urlGenerator->generate('invitemallowancecharge/index', [
                                    'inv_item_id' => $item->getId(),
                                    '_language' => $currentRoute->getArgument(
                                        '_language')],
                                    ['inv_item_id' => $item->getId()],
                                ) ?>" 
                                ...
                                title="<?= $translator->translate(
                                    ...
                                    <i class="<?= $aciiR->repoInvItemCount(
                                        (string) $item->getId()) > 0 ?
                                            'fa fa-list' :
                                                'fa fa-plus'; ?>"></i>
                            </a>
                             <?php } ?>  
                             <a href="
        <?= $urlGenerator->generate('inv/delete_inv_item', [
                                    'id' => $item->getId(),
                                    '_language' => $currentRoute->getArgument(
                                        '_language')]) ?>" 
                             ...
                             <?php if (null !== $item->getTask_id()) { ?>    
                             <a href="
        <?= $urlGenerator->generate('invitem/edit_task', [
                                    'id' => $item->getId(), 
                                    '_language' =>
                            ...
                            <?php if (null !== $item->getProduct_id()) { ?>    
                            <a  href="
        <?= $urlGenerator->generate('invitem/edit_product', [
                                    'id' => $item->getId(), 
                                    '_language' =>
                                    $currentRoute->getArgument('_language')]) ?>" 
                                class="btn btn-success btn">
                ...
                <tr>                        
                    <td>    
                        <div class="input-group">
                            ...
                            <textarea disabled name="item_note"
                                      class="form-control" 
                                      rows="1">
...
"inv_item_amount->subtotal using InvItemController/edit_product->saveInvItemAmount">
    <!-- This subtotal is worked out in
        InvItemController/edit_product->saveInvItemAmount function -->
<?= $numberHelper->format_currency($invItemAmountR->repoInvItemAmountquery(
...
<?php echo $numberHelper->format_currency(
        $invAmount->getItem_subtotal() ?: 0.00); ?>
                    </td>
                </tr>
                <tr>
                    <td>
...
<?php echo $numberHelper->format_currency($invAmount->getItem_tax_total() ?: 0.00); ?>
                    </td>
                </tr>
                <tr>
                    <td>
...
<?= $translator->translate('delete.tax.warning'); ?>');">
                                        <a  href="
<?= $urlGenerator->generate('inv/delete_inv_tax_rate',
        [
