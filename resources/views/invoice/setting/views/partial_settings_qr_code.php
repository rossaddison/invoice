                ...
                <div class="panel-body">
                    <div class="col-xs-12 col-md-6">
                        <div class="form-group">
                            <label for="settings[qr_version]" <?php echo $s->where('qr_version'); ?>>
                                <?php echo $translator->translate('qr.version'); ?>
                            </label>
                            <?php $body['settings[qr_version]'] = $s->getSetting('qr_version') ?: '40'; ?>
                           <input type="text" name="settings[qr_version]" id="settings[qr_version]"
                                class="form-control" 
                                value="<?php echo $body['settings[qr_version]'] ?? (string) Version::AUTO; ?>">
                        </div>
                        <div class="form-group">
                            <label for="settings[qr_ecc_level]">
                                <?php echo $translator->translate('qr.ecc.level'); ?>
                            </label>
                            <?php $body['settings[qr_ecc_level]'] = $s->getSetting('qr_ecc_level'); ?>
                            <select name="settings[qr_ecc_level]" id="settings[qr_ecc_level]"
                                class="form-control">
                                <option value="0" <?php $s->check_select($body['settings[qr_ecc_level]'], '0'); ?>><?php echo 'L'; ?></option>
                                <option value="1" <?php $s->check_select($body['settings[qr_ecc_level]'], '1'); ?>><?php echo 'M'; ?></option>
                                <option value="2" <?php $s->check_select($body['settings[qr_ecc_level]'], '2'); ?>><?php echo 'Q'; ?></option>
                                <option value="3" <?php $s->check_select($body['settings[qr_ecc_level]'], '3'); ?>><?php echo 'H'; ?></option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="settings[qr_height_and_width]" <?php echo $s->where('qr_height_and_width'); ?>>
                                <?php echo $translator->translate('qr.height.and.width'); ?>
                            </label>
                           ...
                           <input type="text" name="settings[qr_height_and_width]" id="settings[qr_height_and_width]"
                                class="form-control" 
                                value="<?php echo isset($body['settings[qr_height_and_width]']) && !empty($body['settings[qr_height_and_width]'])
