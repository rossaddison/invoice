...
<?php
    ...
    <span class="client-address-street-line">
        <?=(strlen($deliveryAddress1 = $delivery_location->getAddress1() ?? '') > 0 ? Html::encode($deliveryAddress1) . '<br>' : ''); ?>
    </span>
    <span class="client-address-street-line">
        <?=(strlen($deliveryAddress2 = $delivery_location->getAddress2() ?? '') > 0 ? Html::encode($deliveryAddress2) . '<br>' : ''); ?>
    </span>
    ...
    <span class="client-adress-country-line">
        <?=(strlen($deliveryCountry = $delivery_location->getCountry() ?? '') > 0 ? '<br>' . $countryHelper->getCountryName($translator->translate('cldr'), $deliveryCountry) : ''); ?>
    </span>
    <br>
    <br>
<?php } ?>
