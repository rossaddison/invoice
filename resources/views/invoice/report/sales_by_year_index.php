...
<div id="headerbar">
    <h1 class="headerbar-title"><?= Html::encode($translator->translate('sales.by.date')); ?></h1>
</div>
                ...
                <div class="panel-body">
                    ...
                    <form method="post" action="<?= $urlGenerator->generate($actionName, $actionArguments); ?>"
                       <?php echo($s->getSetting('open_reports_in_new_tab') === '1' ? 'target="_blank"' : ''); ?>>
                        ...
                        <input type="submit" class="btn btn-success" name="btn_submit" value="<?= $translator->translate('run.report'); ?>">
                    </form>
                </div>
            </div>
        </div>
