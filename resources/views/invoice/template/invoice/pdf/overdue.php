...
$translator->setLocale($cldr);
 ...
 echo H::openTag('body'); //1
    ...
    echo H::openTag('thead'); //4
     echo H::openTag('tr'); //5
      ...
      if ($vat === '0') :
       ...
       echo H::openTag('th', $thItemPrice); //6
        echo H::encode($translator->translate('vat.abbreviation'));
       echo H::closeTag('th'); //6
       echo H::openTag('th', $thItemPrice); //6
        echo '%';
       echo H::closeTag('th'); //6
     ...
     foreach ($items as $item) {
      ...
      foreach ($invItemAllowanceCharges as $invItemAllowanceCharge) {
       echo H::openTag('tr'); //5
        ...
        echo H::tag('td',
         H::encode(($isCharge ? '' : '(') . $s->formatCurrency($amount) . ($isCharge ? '' : ')')),
         $tdEnd);
        $vatInvItem = $invItemAllowanceCharge->getVatOrTax();
        echo H::tag('td',
         H::encode(($isCharge ? '' : '(') . $s->formatCurrency($vatInvItem) . ($isCharge ? '' : ')')),
         $tdEnd);
        echo H::tag('td', '', $tdEnd);
        $p = $invItemAllowanceCharge->getAllowanceCharge()?->getTaxRate()?->getTaxRatePercent();
        echo H::tag('td',
         H::encode($p ?? 0.00),
         $tdEnd);
        echo H::tag('td', '', $tdEnd);
  ...
  echo H::tag('watermarkimage', '',
   ['src' => basename(__FILE__, '.php') . '.png', 'alpha' => '0.1']);
