 ...
 * @var App\Invoice\SalesOrderItemAllowanceCharge\
 *      SalesOrderItemAllowanceChargeRepository $acsoiR
 * @var App\Invoice\SalesOrderItemAmount\
...
<?php
    /**
     * Set the locale when the view is being rendered partially
     * i.e. without a layout
     */
...
<header class="clearfix">    
    ...
    <div id="client">
        <div>
            <b><?= Html::encode(
                $salesorder->getClient()?->getClient_name()
            ); ?></b>
        </div>
        <?php if (strlen(
            $clientVatId = $salesorder->getClient()?->getClient_vat_id() ?? ''
        ) > 0) {
            echo '<div>'
...
if (strlen(
    $clientTaxCode = $salesorder->getClient()?->getClient_tax_code() ?? ''
) > 0) {
    echo '<div>'
...
echo '<div>'
    . Html::encode(
        strlen($salesorder->getClient()?->getClient_address_1() ?? '') > 0
        ?: $translator->translate('street.address')
    )
    . '</div>';
echo '<div>'
    . Html::encode(
        strlen($salesorder->getClient()?->getClient_address_2() ?? '') > 0
        ?: $translator->translate('street.address.2')
    )
    . '</div>';
if (strlen($salesorder->getClient()?->getClient_city() ?? '') > 0
    || strlen($salesorder->getClient()?->getClient_state() ?? '') > 0
    || strlen($salesorder->getClient()?->getClient_zip() ?? '') > 0
) {
    echo '<div>';
    if (strlen($salesorder->getClient()?->getClient_city() ?? '') > 0) {
        echo Html::encode(
            $salesorder->getClient()?->getClient_city()
        ) . ' ';
    }
    if (strlen($salesorder->getClient()?->getClient_state() ?? '') > 0) {
        echo Html::encode(
            $salesorder->getClient()?->getClient_state()
        ) . ' ';
    }
    if (strlen($salesorder->getClient()?->getClient_zip() ?? '') > 0) {
        echo Html::encode(
            $salesorder->getClient()?->getClient_zip()
        );
    }
    echo '</div>';
}
...
if (strlen(
    $clientCountry = $salesorder->getClient()?->getClient_country() ?? ''
) > 0) {
    echo '<div>'
        . $countryHelper->get_country_name(
...
if (strlen(
    $clientPhone = $salesorder->getClient()?->getClient_phone() ?? ''
) > 0) {
    echo '<div>'
        ...
        <table>
            <tr>
                ...
                <td><?php
                    echo Html::encode(
                        !is_string(
                            $dateCreated = $salesorder->getDate_created()
                        )
...
if ($items) {
    ...
    foreach ($items as $item) {
        $salesorder_item_amount = $soiaR->repoSalesOrderItemAmountquery(
            (string) $item->getId()
        );
        ...
        if ($s->getSetting('enable_peppol') == '1') {
            ...
            /**
             * @var App\Invoice\Entity\SalesOrderItemAllowanceCharge
        ...
        ?>
            <tr>
                ...
                <td class="text-right">
                    <?php
                        $vatSalesOrderItem =
                            $salesOrderItemAllowanceCharge->getVatOrTax();
                        echo Html::encode(
                            ($isCharge ? '' : '(')
                            . $numberHelper->format_currency(
                                $vatSalesOrderItem
                ...
                <td class="text-right">
                    <?php
                        $percent = $salesOrderItemAllowanceCharge
                            ->getAllowanceCharge()
                                ?->getTaxRate()
                                    ?->getTaxRatePercent();
        ...
        ?>
            <tr>
                ...
                <td><?php
                    echo nl2br(Html::encode($item->getDescription()));
                ?></td>
                <td class="text-right">
                    <?php
                        echo Html::encode(
                    ...
                    ?>
                    <?php
                        if (strlen($item->getProduct_unit() ?? '') > 0) :
                    ?>
                        <br>
                        <small><?=
                ...
                <td class="text-right">
                    <?php
                        echo Html::encode(
                            $s->format_currency($item->getPrice())
                        );
                    ?>
                </td>
                <?php if ($show_item_discounts) : ?>
                ...
                <td class="text-right">
                    <?php
                        echo Html::encode(
                            $s->format_currency(
                                $salesorder_item_amount?->getTax_total()
                            )
                        );
                    ?>
                </td>
        ...
        <tr>
            ...
            <td class="text-right"><b><?php
                echo Html::encode(
                    $s->format_currency($so_amount->getItem_subtotal())
                );
            ?></b></td>
        </tr>
            ...
            <tr>
                <td <?php
                    ...
                    <?= Html::encode(
                        $vat == '1'
                        ? $translator->translate(
                            'allowance.or.charge.shipping'
                            . '.handling.packaging.vat'
                        )
                        : $translator->translate(
                            'allowance.or.charge.shipping'
                            . '.handling.packaging.tax'
                        )
                    ); ?>
                </td>
                <td class="text-right">
        ...
        <?php
                        /**
                         * @var App\Invoice\Entity\SalesOrderTaxRate
                         *      $salesorder_tax_rate
                         */
                ...
                <td class="text-right">
                    <b><?php
                        echo Html::encode(
                            $s->format_currency(
                                $salesorder_tax_rate
                                    ->getSales_order_tax_rate_amount()
                            )
                        );
                    ?></b>
                </td>
        ...
        <tr>
            ...
            <td class="text-right">
                <b><?php
                    echo Html::encode(
                        $s->format_currency($so_amount->getTotal())
                    );
                ?></b>
            </td>
        </tr>
