 ...
 echo H::openTag('div', ['class' => 'col-12']);
  echo H::openTag('div', ['class' => 'card mb-3']);
   ...
   echo H::openTag('div', ['class' => 'card-body']);
    echo H::tag('p', $translator->translate('homecare.runsheet.index.description'),
        ['class' => 'text-muted']);
        ...
        echo H::openTag('table', ['class' => 'table table-striped table-sm']);
         ...
         echo H::openTag('tbody');
          foreach ($runSheets as $runSheet) {
           $id = $runSheet->reqId();
           echo H::openTag('tr');
            echo H::tag('td', H::a('#' . $id,
                $urlGenerator->generate('homecarerunsheet/review', ['id' => (string) $id]))->render())
                ->encode(false);
