        ...
        <div class="col-12 col-md-8 col-lg-6 col-xl-8">
            <div class="card border border-dark shadow-2-strong rounded-3">
                <div class="card-header bg-dark text-white">
                    <h1 class="fw-normal h3 text-center"><?php echo $translator->translate('email.quote').' #'.($quote->getNumber() ?? '#').' => '.($quote->getClient()?->getClient_email() ?? '#'); ?></h1>
                </div>
...
?>
                    <div class="panel panel-default" hidden>
                        <?php echo Html::tag('Label', $translator->translate('type'), ['for' => 'email_template_type', 'class' => 'control-label']); ?>
                        <?php echo Html::tag(
                            'Div',
                            Html::tag(
                                'Label',
                    ...
                    <?php echo Html::tag('Label', $translator->translate('to.email')); ?>
                    <?php echo Field::email($form, 'to_email')->addInputAttributes(['value' => Html::encode($quote->getClient()?->getClient_email())])
                        ->hideLabel()
                        ->required(true); ?> 
                    
                    <?php echo Html::tag('Label', $translator->translate('email.template')); ?>                        
                    <?php echo Field::select($form, 'email_template')->optionsData($dropdownTitlesOfEmailTemplates, true, [], [])
                        ->disabled(empty($dropdownTitlesOfEmailTemplates) ? true : false)
                        ->hideLabel(); ?>
                    
                    <?php echo Html::tag('Label', $translator->translate('from.name')); ?>
                    <?php echo Field::text($form, 'from_name')->addInputAttributes(['class' => 'email-template-from-name form-control'])
                        ->addInputAttributes(['value' => $autoTemplate['from_name'] ?? '' ?: Html::encode($userInv->getName())])
                        ->hideLabel()
                        ->required(true); ?>
                    
                    <?php echo (string) Html::tag('Label', $translator->translate('from.email')).str_repeat('&nbsp;', 2).($autoTemplate['from_email'] ? $translator->translate('email.source.email.template') : $translator->translate('email.source.user.account')); ?>
                    <?php echo Field::email($form, 'from_email')->addInputAttributes(['value' => $autoTemplate['from_email'] ?? '' ?: Html::encode($userInv->getUser()?->getEmail())])->hideLabel()
                        ->addInputAttributes(['class' => 'email-template-from-email form-control'])
                        ->required(true); ?>                            
                    
                    <?php echo Html::tag('Label', $translator->translate('cc')); ?>
                    <?php echo Field::text($form, 'cc')->addInputAttributes(['class' => 'email-template-cc form-control'])
                        ->addInputAttributes(['value' => $autoTemplate['cc'] ?? ''])
                        ->hideLabel(); ?>
                    
                    <?php echo Html::tag('Label', $translator->translate('bcc')); ?>
                    <?php echo Field::email($form, 'bcc')->addInputAttributes(['class' => 'email-template-bcc form-control'])
                        ->addInputAttributes(['value' => $autoTemplate['bcc'] ?? ''])
                        ->hideLabel(); ?>
                                        
                    <?php echo Html::tag('Label', $translator->translate('subject')); ?>
                    <?php echo Field::text($form, 'subject')->addInputAttributes(['id' => 'mailerquoteform-subject'])
                    ...
                    <?php echo Html::tag('Label', $translator->translate('pdf.template')); ?>
                    <?php echo Field::select($form, 'pdf_template')->optionsData($pdfTemplates, true, [], [])
                    ...
                    <?php echo Html::tag('Label', $translator->translate('body')); ?>
                    
                    <?php echo Field::textarea($form, 'body')->addInputAttributes(['id' => 'mailerquoteform-body'])
...
?>
                    ...
                    <?php echo Field::file($form, 'attachFiles[]')
                        ->containerClass('mb-3')
                        ->multiple()
                        ->hideLabel();
?>                   
                    <div>
                    <div class="form-group"><?php echo Html::tag('Label', $translator->translate('guest.url'), ['for' => 'quote-guest-url']); ?></label>
                        <div class="input-group">
                        <?php echo Field::text($form, 'guest_url')->readonly(true)
