    ...
    <div class="modal-dialog">
        <div class="modal-content">
           <div class="modal-header">
               <h5 class="modal-title"><?= $translator->translate('delete.quote'); ?></h5>
               <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-danger"><?= $translator->translate('delete.quote.warning'); ?></div>
                <form action="<?= $urlGenerator->generate($actionName, $actionArguments) ?>" method="POST">
                    <input type="hidden" name="_csrf" value="<?= $csrf ?>">
                    <div class="btn-group">
                        <button type="submit" class="btn btn-danger">
                            <i class="bi bi-trash me-1"></i> <?= $translator->translate('confirm.deletion') ?>
                        </button>
