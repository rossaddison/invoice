...
namespace <?php echo $generator->getNamespace_path().'\Entity'; ?>;
...
<?php
    ...
    foreach ($orm_schema->getColumns() as $column) {
        ...
        if ('datetime' === $column->getAbstractType()) {
...
class <?php echo $generator->getCamelcase_capital_name()."\n"; ?>
{
    <?php
       /**
         * @var App\Invoice\Entity\GentorRelation $relation
...
foreach ($orm_schema->getColumns() as $column) {
    ...
    if ('boolean' === $column->getAbstractType()) {
    ...
    if (('date' != $column->getAbstractType()) && ('datetime' != $column->getAbstractType()) && ('boolean' != $column->getAbstractType())) {
...
?>
     ...
     */
    public function nullifyRelationOnChange(<?php echo $final_string; ?>) : void 
    {
        <?php
    /**
     * @var App\Invoice\Entity\GentorRelation $relation
