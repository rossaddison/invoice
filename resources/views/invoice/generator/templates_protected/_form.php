...
/**
 * @var App\Invoice\<?php echo $generator->getCamelcase_capital_name(); ?>\<?php echo $generator->getCamelcase_capital_name(); ?>Form $form
 ...
 * @psalm-var array<array-key, array<array-key, string>|string> $<?php echo $generator->getSmall_singular_name(); ?>         
 */
...
foreach ($orm_schema->getColumns() as $column) {
    ...
    if ('_id' != substr($column->getName(), -3)) {
         ...
         */
        if (('mixed' === $column->getType()) && ('date' === $column->getAbstractType())) {
            echo "<?= Html::openTag('div'); ?>";
         ...
         */
        if (('float' === $column->getType()) && ('decimal' === $column->getAbstractType())) {
            echo "<?= Html::openTag('div'); ?>";
         ...
         */
        if (('string' === $column->getType()) && ('date' != $column->getAbstractType())) {
            echo "<?= Html::openTag('div'); ?>";
            echo '<?= Field::text($form,'."'".$column->getName()."')";
            echo '    ->label($translator->translate('.$column->getName().'))';
            echo '    ->addInputAttributes([';
        ...
        if (('int' === $column->getType()) && ('date' != $column->getAbstractType()) && ('primary' != $column->getAbstractType())) {
            echo "<?= Html::openTag('div'); ?>";
            echo '<?= Field::text($form,'."'".$column->getName()."')";
            echo '    ->label($translator->translate('.$column->getName().'))';
            echo '    ->addInputAttributes([';
