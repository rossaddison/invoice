...
1 == $s->getSetting('monospace_amounts') ? $assetManager->register(MonospaceAsset::class) : '';
// '0' => PCI Compliant version
'0' == $s->getSetting('gateway_stripe_version') ? $assetManager->register(stripe_v10_Asset::class) : '';
'0' == $s->getSetting('gateway_amazon_pay_version') ? $assetManager->register(amazon_pay_v2_4_Asset::class) : '';
'0' == $s->getSetting('gateway_braintree_version') ? $assetManager->register(braintree_dropin_1_33_7_Asset::class) : '';
$vat = ('0' == $s->getSetting('enable_vat_registration'));
...
<html class="h-100" lang="<?php echo $currentRoute->getArgument('_language') ?? 'en'; ?>">
    <head>
        <?php echo Meta::documentEncoding('utf-8'); ?>
        <?php echo Meta::pragmaDirective('X-UA-Compatible', 'IE=edge'); ?>
        <?php echo Meta::data('viewport', 'width=device-width, initial-scale=1'); ?>
        <title>
            <?php echo $s->getSetting('custom_title') ?: 'Yii-Invoice'; ?>
        </title>
        <?php $this->head(); ?>
    </head>
    <body>
            ...
            ->items(
                ...
                Dropdown::widget()
                    ...
                    ->items(
                        ...
                        DropdownItem::text('php.ini (line 425): max_execution_time (pref 360) = '.((string) ini_get('max_execution_time') ?: 'unknown').(360 == (string) ini_get('max_execution_time') ? '✅' : '❌')),
                        ...
                        DropdownItem::text('php.ini: (line 1791): opcache.enable (pref 1) = '.((string) ini_get('opcache.enable') ?: 'unknown').(1 == (string) ini_get('opcache.enable') ? '✅' : '❌')),
                        DropdownItem::text('php.ini (line 1794): opcache.enable_cli (pref 1) = '.((string) ini_get('opcache.enable_cli') ?: 'unknown').(1 == (string) ini_get('opcache.enable_cli') ? '✅' : '❌')),
                        DropdownItem::text('php.ini (line 1797): opcache.memory_consumption (pref 128) = '.((string) ini_get('opcache.memory_consumption') ?: 'unknown').(128 == (string) ini_get('opcache.memory_consumption') ? '✅' : '❌'), ['data-bs-toggle' => 'tooltip', 'title' => 'e.g. change manually in C:\wamp64\bin\php\php8.1.13\phpForApache.ini and restart all services.']),
                        DropdownItem::text('php.ini (line 1800): opcache.interned_strings_buffer (pref 64 for frameworks) = '.((string) ini_get('opcache.interned_strings_buffer') ?: 'unknown').(64 == (string) ini_get('opcache.interned_strings_buffer') ? '✅' : '❌')),
                        DropdownItem::text('php.ini (line 1804): opcache.max_accelerated_files (pref 4000) = '.((string) ini_get('opcache.max_accelerated_files') ?: 'unknown').(4000 == (string) ini_get('opcache.max_accelerated_files') ? '✅' : '❌')),
                        DropdownItem::text('php.ini: (line 1818): opcache.validate_timestamps (pref 0) = '.((string) ini_get('opcache.validate_timestamps') ?: 'unknown').(0 == (string) ini_get('opcache.validate_timestamps') ? '✅' : '❌')),
                        DropdownItem::text('php.ini: (line 1822): opcache.revalidate_freq (pref 60) = '.((string) ini_get('opcache.revalidate_freq') ?: 'unknown').(60 == (string) ini_get('opcache.revalidate_freq') ? '✅' : '❌')),
                        DropdownItem::divider(),
                        ...
                        DropdownItem::text('php.ini (line 451): memory_limit (pref 1024 M) = '.((string) ini_get('memory_limit') ?: 'unknown').('1024M' == (string) ini_get('memory_limit') ? '✅' : '❌')),
                        DropdownItem::divider(),
        ...
        ->items(
            ...
            Dropdown::widget()
                ...
                ->items(
                    ...
                    DropdownItem::link($translator->translate((('1' == $s->getSetting('install_test_data')) && ('1' == $s->getSetting('use_test_data')))
                    ? 'install.test.data' : 'install.test.data.goto.tab.index'), ('1' == $s->getSetting('install_test_data') && '1' == $s->getSetting('use_test_data'))
                    ? $urlGenerator->generate('invoice/index') : $urlGenerator->generate('setting/tab_index'), '1' == $s->getSetting('install_test_data') && '1' == $s->getSetting('use_test_data')),
            ...
            <div id="fullpage-loader" style="display: none">
                ...
                <div class="text-right">
                    <button type="button" class="fullpage-loader-close btn btn-link tip" aria-label="<?php $translator->translate('close'); ?>"
                            title="<?php echo $translator->translate('close'); ?>" data-placement="left">
