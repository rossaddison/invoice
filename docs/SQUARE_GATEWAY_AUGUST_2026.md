...
`order_id` and this app's own invoice `url_key` are **two separate
values** — not the same field under different names. They're bridged
through a third field, Square's own `reference_id`, and the two only meet
in one direction each:

- **This app → Square**, at Payment Link creation:
  `SquarePaymentService::createPayment()` sends the invoice's `url_key` to
  Square as the Order's `reference_id` — a caller-supplied label Square
  just stores and echoes back on request. (Only available via the fuller
  Order-based request shape, not Quick Pay's simpler ad hoc item — the
  reason Payment Links are built the longer way here.)
- **Square → this app**, on the webhook: the Payment webhook payload
  carries Square's own opaque `order_id` — generated by Square, unrelated
  to anything this app sent — but it never echoes `reference_id` back
  directly.
- **The bridge**: because the webhook only ever hands back `order_id`,
  `SquareWebhookHandler` makes a **second** call,
  `getOrderReferenceId()` (`GET /v2/orders/{id}`), and reads `reference_id`
  back out of *that* response — which is the invoice's `url_key`.

A similar shape to Razorpay's payment-link-id-vs-payment-id wrinkle, just
bridged by an extra API call rather than two IDs carried in the same
payload (this part unchanged since it first shipped).
