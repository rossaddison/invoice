 ...
 * rule: a new invoice is only generated for a client when their last paid
 * invoice's payment date is on file and no invoice at all (paid or not) has
 * been dated since.
 */
#[Test]
final class HomeCareCleaningEligibilityServiceTest
{
    public function noInvoicesAtAllIsNotEligible(): void
    {
        ...
        $service = new HomeCareCleaningEligibilityService($invR, $paymentR);
    ...
    public function paidWithNoInterimInvoiceIsEligibleAndReturnsLastPaidInvoice(): void
    {
        $lastPaid = m::mock(Inv::class);
        /** @var \Mockery\Expectation $e */
        $e = $lastPaid->shouldReceive('reqId');
