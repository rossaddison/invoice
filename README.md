...
[Batch Email — Send Selected Invoices to Clients](docs/INV_BATCH_EMAIL.md) — ☑️📧 **Email Client** toolbar button on `inv/index`; select invoices via checkboxes, confirmation modal shows per-client email resolution (UserClient entity → User email, else Client email), choose an email template, confirm → one email per client with all PDFs attached, each invoice marked Sent (status 2), `InvSentLog` entries created; `{{{invoice_table}}}` placeholder renders a responsive HTML table (Invoice #, Date, Total, Balance, Note, Guest URL); `MailerHelper::yiiMailerSend` signature changed from `?string` to `array $pdfPaths` supporting multiple attachments; `InvBatchEmailDeps` value object avoids SonarQube S107; `user_name` variable added to `TemplateHelper::resolveUserVar()`; Psalm errorLevel 1 clean (June 2026)
