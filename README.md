...
[A Speculative PINT-UK Plan](docs/PEPPOL_PINT_UK_SPECULATIVE_PLAN.md) — Explicitly speculative (no official PINT-UK exists yet — UK confirmed Peppol 4-corner, no e-reporting, on 23 June 2026, but the document profile itself awaits the November 2026 Budget roadmap); extrapolates from every real PINT variant (A-NZ, SG, MY, JP, AE, EU) applied to actual UK VAT specifics: tax scheme needs no relabeling (already "VAT"), the `9932` "United Kingdom VAT number" ISO 6523 ICD code is already present in this repo's own `eas.xml`, but UK VAT's five real tax categories (Standard/Reduced/Zero-rated/Exempt/Outside-scope) plus domestic reverse charge and Postponed VAT Accounting are structurally closer to Malaysia's multi-tax-type problem than Australia's single-scheme swap — confirming `PeppolProfile` needs `taxCategories: array<TaxCategorySpec>`, not a scalar; flags Northern Ireland's Windsor Framework goods alignment as a wrinkle no other PINT variant has; proposes designing the tax-category vocabulary jointly with `vat-api-php`'s VAT100 box mapping rather than as two separate efforts (July 2026)

[Adapting Peppol Output to PINT A-NZ](docs/PEPPOL_PINT_AUNZ_ADAPTATION_PLAN.md) — Unlike the UK mandate, PINT A-NZ has been the only supported Peppol invoice format for Australia/New Zealand since 15 May 2025 — live and mandatory today, with a free public validator; code-grounded scope found by checking (not guessing) the actual source: `DocumentLevelValidator::validateCustomizationID()` would currently **reject** a valid PINT A-NZ document since it hardcodes the core-BIS-3.0 URN prefix, `PeppolValidator`'s `XPATH_TAX_SCHEME_VAT` hardcodes the literal `'VAT'` so it wouldn't even locate a GST-labelled tax ID, and `PeppolHelper::TAX_CATEGORY_VAT` plus `Invoice.php`'s `CustomizationID` default are both hardcoded constants; proposes a `PeppolProfile` enum threading profile-aware CustomizationID/ProfileID/tax-scheme/identifier-scheme values through both validators and the XSLT rendering stylesheet; ~10 hour estimate; deliberately scoped to document generation only, not real AS4 transmission to an ATO-accredited Access Point (July 2026)

[Deterministic Keyword Command Router — Free, Non-LLM MCP Front Door](docs/KEYWORD_COMMAND_ROUTER_PLAN.md) — Plan for a second, independent way to trigger `InvoiceCapabilitiesInterface` methods (the same interface the MCP server uses) via fixed regex/keyword patterns instead of an LLM — no Claude, no API cost, no dependency on a service that may be blocked or unaffordable for self-hosters in some countries; wires into the existing (currently outbound-only) `TelegramController` webhook, resolving the sending chat to a `UserInv` via its already-present but currently-unused `telegram_chat_id` column and enforcing the same `Permissions::hasPermission()` checks the web controllers use per intent; 9-intent grammar table (list/get/create invoice, send via Peppol, Peppol status, list/get client, list purchase entries, help) with a fixed "not understood" fallback — no fuzzy matching or LLM fallback by design; `telegram/route-test` console command lets any contributor validate the grammar offline with no network or bot token; MCP and this router are two independent front doors onto the same backing capability class (July 2026)

[MCP and Voice Accessibility — Why It Matters for Users with Disabilities](docs/MCP_ACCESSIBILITY.md) — Explains what the Model Context Protocol is in plain terms and how the existing Yii3-i MCP server (`invoice_list`, `invoice_create`, `invoice_send_peppol`, etc.) lets people with motor/dexterity impairments, blindness/low vision, chronic pain, or high cognitive load drive the full invoicing workflow by speaking through a microphone instead of navigating GridViews, toolbars, and modals; dictation via Windows Voice Access / macOS Voice Control / phone mic already works today with zero extra integration; Phase 2 Angular `AiAssistantComponent` will bring a mic button directly into the web UI (July 2026)

[MCP Server — Yii3-i as an AI-Accessible Invoice Service](docs/MCP_SERVER_PLAN.md) — PHP MCP server (`mcp/sdk`) exposes invoice creation and Peppol send path as MCP tools callable from Claude Desktop, Claude.ai, and Cursor; `McpServeCommand` (stdio) + HTTP `/mcp` endpoint; `InvoiceCapabilities` bridges Yii3 DI to the SDK's tool registry; Phase 2 Angular MCP client from 19 July 2026 using `@modelcontextprotocol/client` v2 stable (July 2026)
