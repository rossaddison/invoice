...
[Security Hardening Audit](docs/SECURITY_HARDENING_AUDIT_JULY_2026.md) — Static/config-level review of the app's security posture (no dynamic testing). Critical: unrestricted file upload → web root in `CompanyPrivateController::processLogoFileUpload()` and `ProductAttachmentController::imageAttachment()` (no extension/MIME validation, writes into `public/logo/` and `public/products/`); session cookie missing the `Secure` flag (`yiisoft/session` package default of `cookie_secure => 0` never overridden). High: no rate limiting on the public `/scan/{token}` HomeCare QR endpoint, unlike every other public auth route; hardcoded `yiisoft/cookies` signing secret committed in `config/common/params.php`. Medium: no HSTS/Permissions-Policy header, other security headers only enforced in `public/.htaccess` not at the PHP layer; CSP allows `unsafe-inline`/`unsafe-eval`; 2FA opt-in with no mandatory role and no per-account lockout. Low: one unescaped `Html::encode()` miss in `modal_change_client.php`; RBAC enforcement is fail-open (opt-in per route). Confirmed solid: CSRF, bcrypt password hashing, CSPRNG tokens, existing auth-route rate limiting, `.env` secret hygiene, Renovate dependency automation. Fixes not yet started — this is the punch list (July 2026)
