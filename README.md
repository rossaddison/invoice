...
[Dev Tools Web UI — `m.bat` / `m.php`](docs/DEV_TOOLS_WEB_UI.md) — `m.bat` replaced by a PHP built-in web server (`php -S 127.0.0.1:8099 m.php`) eliminating all batch-file stdin issues; 16 category submenus (Psalm, Composer, Node, TypeScript, Angular, Testing, Snyk, PHP-CS-Fixer, PHPCS, Rector, SonarCloud, Yii, GitHub, Peppol, Benchmarks, System); streaming output via `proc_open()` + `ReadableStream`; ANSI colour rendering in browser (foreground codes 30–97, background codes 40–107, underline); OSC 8 terminal hyperlinks rendered as clickable HTML anchors (e.g. `composer outdated` package links open GitHub pages); Bootstrap 5.3 dark theme; session-stored SonarCloud/GitHub tokens; Snyk Resolved Vulnerabilities Index (SQLite, seeded from `.snyk`, committed to repo, CWE advisory links); SQLite setup guide distinguishing CLI PHP from WAMP Apache PHP; 16 local SVG menu icons (Simple Icons for brands, Bootstrap Icons for generics, official Yii3 logo in brand colours) served statically via built-in server passthrough; Bootstrap 5 hover/focus popovers on each category card list every submenu command for discoverability; **Copy** button on output panel writes plain text to clipboard with green flash confirmation; SonarCloud *Filter by Rule Key* uses live cascading dropdowns — language selector triggers a `?api=failing_rules` fetch that runs `sonar-issues.php --grouped`, parses currently-failing `lang:S####` issues, and populates the rule-number dropdown with only the rules that are actually failing for that language (June 2026)
