...
[Credit Note — Allowance/Charge Propagation & UBL Document Type Fix](docs/CREDIT_NOTE_ALLOWANCE_CHARGE_AND_UBL_TYPE_FIX_JULY_2026.md) — a credit note needs to stand on its own as a legal/Peppol document, but two gaps meant it was incomplete or mislabeled: if the original invoice had line-level (`InvItemAllowanceCharge`) or document-level (`InvAllowanceCharge`) discounts/surcharges, `Trait\Credit::processCreditConfirm()` never propagated them onto the credit note (unlike the regular copy-invoice flow, which already did, negated to match); and the outbound UBL/Peppol XML was always generated as an `<Invoice>` document with `InvoiceTypeCode 380`, never the Peppol-required `<CreditNote>`/`381` — `Ubl\Generator::creditNote()` existed but was dead code that even then hardcoded the wrong root tag/namespace. Fixed by threading a `PeppolHelper::isCreditNote()` check (resolved from the invoice's group) through `PeppolUblXml`/`Ubl\Generator` to emit the correct root element, namespace, type code, and `CreditNoteLine`/`CreditedQuantity` line elements, and by adding `InvAllowanceChargeService::initializeCreditInvAllowanceCharges()` plus a multiplier on `InvItemService::addInvItemAllowanceCharges()` to negate and copy both allowance/charge levels onto the credit note. Verified end-to-end against real dev-DB data and a new test that runs a full document graph through the actual XML pipeline. Remaining: credit note amounts are stored negative internally and currently flow into the UBL output unchanged — converting to positive magnitudes at the XML boundary (to match standard Peppol CreditNote convention) is a deliberately separate follow-up. Psalm clean throughout (July 2026)
